function extract_CodeWords_BatchOfSongs(numCodeWordsPerBatch, songsPerBatch, iter)
%{
	Script that does the following:
	+ Randomly select 10 songs files's NORM_DYN_MFCC
...
while (fileCount < length(Files) + 1)
    ...
    while(mod(fileCount, songsPerBatch) ~=0 && fileCount <= length(Files) )
        ...
        if (Files(songID).bytes > 0 )
            ...
            BATCH_NORM_DYN_MFCC = [BATCH_NORM_DYN_MFCC NORM_DYN_MFCC];
    ...
    size(BATCH_NORM_DYN_MFCC)
    tic
    BATCH_CODEWORDS = ma_kmeans(BATCH_NORM_DYN_MFCC', iter, numCodeWordsPerBatch);
    toc
